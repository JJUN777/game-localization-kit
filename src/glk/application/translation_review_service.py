...
_FINAL_HEADER = "[[GLK_TRANSLATION_FINAL version=1]]"
_SEPARATOR = "======================"
...
def _parse_review_text(
    ...
    try:
        ...
        raise TranslationReviewParseError(
            "invalid_encoding", "Translation review TXT must be valid UTF-8."
        ) from error
    if not lines or lines[0] != _REVIEW_HEADER:
        raise TranslationReviewParseError(
            "invalid_header",
            "Translation review header is missing or unsupported.",
        )
    ...
    for segment in segments:
        ...
        if current_locator != previous_locator:
            ...
            if found != expected:
                raise TranslationReviewParseError(
                    "location_changed",
                    (
                        f"Location marker for block {segment.source_block_id} changed; "
        ...
        if found != expected_block:
            ...
            raise TranslationReviewParseError(
                "block_order_changed",
                (
                    f"Block marker order is invalid; expected "
                    f"{segment.source_block_id}, {detail}."
                ),
                segment.source_block_id,
            )
        index += 1
        if index >= len(lines) or lines[index] != "[ORIGINAL]":
            raise TranslationReviewParseError(
                ...
                f"[ORIGINAL] marker for block {segment.source_block_id} changed.",
                segment.source_block_id,
            )
        index += 1
        ...
        if found_source_lines != expected_source_lines:
            raise TranslationReviewParseError(
                "source_changed",
                (
                    f"Original source for block {segment.source_block_id} changed. "
                    "Only edit text below [TRANSLATION]."
                ),
                segment.source_block_id,
            )
        ...
        if index >= len(lines) or lines[index] != "[TRANSLATION]":
            raise TranslationReviewParseError(
                ...
                f"[TRANSLATION] marker for block {segment.source_block_id} changed.",
                segment.source_block_id,
            )
        index += 1
        ...
        try:
            ...
            raise TranslationReviewParseError(
                "end_marker_changed",
                f"End marker for block {segment.source_block_id} changed or was removed.",
                segment.source_block_id,
            ) from error
    ...
    if index != len(lines):
        ...
        message = (
            f"Unknown or duplicate block in review TXT: {match.group(1)}"
            if match
            else f"Unexpected content after the final block: {found!r}"
        )
        ...
        if "�" in translated:
            issues.append(
                TranslationReviewIssue(
                    ...
                    block_id=segment.source_block_id,
                    message="Translation contains a Unicode replacement character.",
                )
            )
        if "[ILLEGIBLE]" in translated.upper():
            issues.append(
                TranslationReviewIssue(
                    ...
                    block_id=segment.source_block_id,
                    message="Translation still contains [ILLEGIBLE].",
                )
            )
        ...
        if translated == segment.source_text and _LATIN_PATTERN.search(
            ...
            issues.append(
                TranslationReviewIssue(
                    ...
                    block_id=segment.source_block_id,
                    message="Korean target text contains no Hangul.",
                )
            )
...
def get_project_translation_review_document(
    *,
    ...
    if not translations:
        ...
        detail = issue.message if issue else "The review TXT could not be parsed."
        raise TranslationReviewError(
