...
class GlossaryCandidateRuleTests(unittest.TestCase):
    ...
    def test_filters_quantity_prefixes_partial_titles_and_generic_words(self) -> None:
        blocks = [
            make_block(1, "4x Fire terrain token", block_type="label"),
            make_block(2, "Attrition deck (10x cards)", block_type="label"),
            make_block(3, "Hit the Tail! objective card", block_type="label"),
            make_block(
                4,
                "Other components are used in the Campaign and Expedition modes.",
            ),
            make_block(5, "These cards display the following information."),
            make_block(6, "Other cards display the following icon."),
            make_block(
                7,
                "3. Behavior deck: This deck determines the monster behavior.",
                block_type="list_item",
            ),
            make_block(8, "Bonus damage token", block_type="label"),
            make_block(9, "Bonus damage token", block_type="label"),
            make_block(10, "THE MONSTER", block_type="heading"),
            make_block(11, "Stance III and III cards."),
            make_block(12, "2d. Health track", block_type="list_item"),
        ]

        candidates = extract_glossary_candidates(blocks, min_frequency=2)
        by_key = {candidate.source_term.casefold(): candidate for candidate in candidates}

        self.assertIn("fire terrain token", by_key)
        self.assertEqual(by_key["fire terrain token"].category, "component")
        self.assertIn("attrition deck", by_key)
        self.assertIn("hit the tail", by_key)
        self.assertIn("behavior deck", by_key)
        self.assertIn("bonus damage token", by_key)
        self.assertIn("health track", by_key)
        self.assertEqual(by_key["health track"].category, "component")
        self.assertNotIn("x fire terrain token", by_key)
        self.assertNotIn("x cards", by_key)
        self.assertNotIn("campaign and", by_key)
        self.assertNotIn("and expedition", by_key)
        self.assertNotIn("display", by_key)
        self.assertNotIn("following", by_key)
        self.assertNotIn("the monster", by_key)
        self.assertNotIn("iii", by_key)
        self.assertNotIn("bonus", by_key)
        self.assertNotIn("bonus damage", by_key)
