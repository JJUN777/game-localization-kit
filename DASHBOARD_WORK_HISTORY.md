...
**현재 범위**: 승인 원문 기반 용어 후보 생성 UI 완료 — 다음 작업은 용어 검수 완료 후 대시보드 복귀
...
### 다음 작업: 용어 검수 완료 후 대시보드 복귀

GUI 3.1에서 원문 준비 background job과 원문 검수 진입까지 연결했습니다.
원문 검수의 `최종 승인` 뒤에는 완료 모달에서 대시보드로 복귀할 수 있습니다.
승인 상태에서는 로컬 background job으로 용어 후보를 생성하고 기존 용어 검수
화면으로 진입할 수 있습니다. 다음에는 용어 검수 화면의 `검증 및 termbase
생성` 성공 뒤 완료 모달과 대시보드 복귀를 추가합니다.
