...
**현재 범위**: GUI 3.1 원문 준비 오류 안내 개선 완료 — 다음 작업은 원문 승인 완료 화면과 용어 후보 생성 UI 연결
...
### 다음 작업: 원문 승인 완료와 용어 후보 생성 UI 연결

GUI 3.1에서 원문 준비 background job과 원문 검수 진입까지 연결했습니다.
원문 검수의 `최종 승인`은 현재 confirm과 성공 toast 뒤 같은 화면에 머뭅니다.
다음에는 승인 완료 모달에서 `대시보드로 돌아가기`와 `이 화면에 머물기`를
선택하게 하고, 대시보드로 복귀하면 승인 상태를 새로 읽어 `용어 후보 생성`
버튼을 활성화합니다. 후보 생성 뒤에는 기존 용어 검수 화면으로 이어집니다.
장시간 번역 job과 cooperative cancellation은 후속 범위로 유지합니다.
