...
5. `원문 검수`에서 원본과 추출문을 비교·수정하고 최종 승인합니다.
   ...
   승인 뒤 프로젝트 카드의 `파일 저장` 영역에서 `승인 원문`을 저장할 수 있습니다.
6. `용어 후보 생성 시작` 후 `용어 검수`에서 번역어를 확정합니다. 필요하면
   `후보 1차 정리(AI)`로 아직 검토하지 않은 자동 후보의 추천을 먼저 확인합니다.
7. 카드의 `설정 > 번역 프롬프트 설정`에서 번역 지침을 필요에 따라 수정합니다.
   한 번 저장한 뒤에는 메뉴명이 `번역 프롬프트 수정`으로 바뀝니다.
   `AI로 초안 만들기`를 선택하면 승인 원문 대표 문장으로 문체 지침을 제안하며,
   예상·실제 token과 비용을 확인한 뒤 사용할 수 있습니다.
   프롬프트를 저장하고 번역을 시작합니다.
8. `번역 검수`에서 QA 항목을 수정하고 최종 승인합니다.
9. 프로젝트 카드의 `파일 저장`에서 PDF 최종 번역본 TXT 또는 이미지 통합
   번역본 TXT·이미지별 번역본 ZIP을 저장합니다.
...
`설정 > OCR 프롬프트 수정`(GUI) 또는
`01_input/images/ocr_prompt.txt`(CLI)에서
...
GUI: 완료된 청크가 있으면 카드의 `초벌 번역 이어서 실행`을 사용합니다.
저장된 청크가 없으면 `초벌 번역 시작`으로 다시 실행합니다.
