...
> 상태: 2026-07-23 시점의 상세 작업 기록입니다. 현재 이어서 할 작업은 [인수인계](../project/handoff.md)를 기준으로 합니다.

이 문서는 다른 컴퓨터나 새 세션에서 현재 구현 상태를 빠르게 확인하고 같은 지점부터 작업하기 위한 인수인계 기록입니다. 현재 실행 순서는 [전체 작업 흐름](../guides/workflow.md), 당시 세부 설계는 [번역 자동화 파이프라인 설계](TRANSLATION_AUTOMATION_DESIGN_DRAFT.md), 당시 체크리스트는 [개선 로드맵](IMPROVEMENTS_20260723.md)을 함께 참고합니다.

용어집 단계의 사람이 편집하는 TSV 구조와 수동 용어 추가 방식은 [용어집 검토 사양](../guides/glossary.md)으로 확정했습니다.
