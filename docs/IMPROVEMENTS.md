...
- [ ] `ARCH-006` 상태 판정·I/O·provider 호출이 다시 섞인 대형 use case를
  ...
  - PDF·OCR·원문 검수 저장·선택 재번역 단계 검증: 전체 291개 테스트와
    mypy 48개 파일·ruff 검사를 통과했다.
...
1. `ARCH-006`: 선택한 use case를 각각 독립 커밋으로 분리
2. `DOMAIN-001`: 하이픈 결합 경고의 검수 화면 표시 방법 확정과 구현

각 묶음은 관련 회귀 테스트, 전체 테스트, 필요한 경우 Orca 브라우저 검증을
통과해야 완료로 표시합니다.
세 작업을 모두 마친 뒤 [`BACKLOG.md`](BACKLOG.md)의 후속 항목 우선순위를
다시 결정합니다.
