...
완료 상태는 이 문서만 갱신합니다.
현재 안정화 범위 뒤에 검토할 제품 기능·실측·정책 항목은
[`BACKLOG.md`](BACKLOG.md)에 분리해 보관합니다.
...
- [x] `DOMAIN-001` 줄바꿈 하이픈 결합 결과를 원문 검수에서 확인할 수 있게 한다.
  - 단어 사전 기반 자동 교정보다 결합 경고와 원문 비교를 우선한다.
  - PDF fragment 경계에서 `multi-` + `player`처럼 소문자로 이어져 하이픈을
    제거한 지점을 source block의 추출 경고와 `flagged` 상태로 보존한다.
  - 원문 검수 화면은 QA 실행 여부와 무관하게 경고 개수, 카드 배지와 결합 전후
    근거를 표시해 사용자가 원본 페이지와 비교한 뒤 직접 수정할 수 있게 한다.
  - 검증: fragment 근거 보존과 browser document 전달 회귀 테스트를 포함한 전체
    293개 테스트, mypy 48개 파일과 ruff 검사를 통과했다. Orca 브라우저에서
    경고 집계·배지·결합 근거 표시와 콘솔 오류가 없음을 확인했다.
...
- [x] `ARCH-006` 상태 판정·I/O·provider 호출이 다시 섞인 대형 use case를
  작은 단계로 분리한다.
  ...
  - [x] `extract_project_pdf`: 페이지 하나의 fragment·cache·provider·산출물
    ...
    부분 실패 계약을 유지한다.
  - [x] `translate_project`: 입력·cache/checkpoint 복원 뒤 기존 검증 경고와
    초기 상태를 준비하는 단계, 미완료 청크의 provider 호출·append·부분 상태
    기록, 최종 draft·review·완료 상태 저장을 분리했다. 진입 함수는 191줄에서
    66줄로 줄었고 cache 재사용·부분 실패·재개·stale·사람 검수본 보존 계약을
    유지한다.
  ...
  - [x] `retry_failed_translations`: 검수·QA 대상 준비, dry-run 결과, 블록별
    provider 호출과 2회 검증, 전체 변경 집계, optimistic lock 저장·revision·
    QA를 분리했다. 진입 함수는 184줄에서 58줄로 줄었고 선택 블록만 교체,
    draft 보존, dry-run 무호출과 검증 실패 시 review 무변경 계약을 유지한다.
  - 완료 기준: 우선 범위의 각 진입 함수가 120줄 이하이고, 추출한 단계별
    helper에 성공·부분 실패·재개·stale 회귀 테스트가 있어야 한다. 줄 수를
    맞추기 위한 의미 없는 wrapper 분리는 허용하지 않는다.
  - PDF·OCR·초벌 번역·원문 검수 저장·선택 재번역 단계 검증: 전체 291개
    테스트와 mypy 48개 파일·ruff 검사를 통과했다.
...
- [x] `ERROR-003` 예외 문구 부분 검색에 의존하는 오류 판정을 도메인별
  안정적 코드로 교체한다.
  ...
  - 2차 전환: PDF·OCR 실패 레코드에 안정적인 provider 코드를 저장하고,
    dashboard job과 번역 실패 안내는 SDK 상태·도메인 예외 타입만으로
    결정한다. 공통 HTTP 응답의 detail fallback을 제거하고 프로젝트·OCR
    프롬프트·용어 import의 기존 행동 안내를 전용 코드로 보존했다.
  - 최종 검증: 숫자가 섞인 일반 예외를 provider 오류로 오인하지 않고, 코드가
    없는 과거 acquisition 상태는 안전한 일반 안내로 복원됨을 확인했다.
    전체 291개 테스트와 mypy 48개 파일·ruff 검사를 통과했다.
