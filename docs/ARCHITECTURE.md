...
3. LLM은 OCR, 레이아웃 판정, 명시적으로 선택한 PDF 아이콘 검사와 초벌 번역에
   사용하고, 구조·hash·token 검사는 로컬 코드가 담당한다.
...
**원본 교체:** 원문 추출·OCR 전에는 항상 허용합니다. 기존 PDF·이미지 입력
...
트랜잭션 안에서 원자적으로 저장합니다. PDF acquisition이 `partial`이어도 원문
검수 block과 후속 사람 작업이 아직 없다면 교체를 허용하고, acquisition cache와
job 상태처럼 재생성 가능한 자료만 정리합니다. 검수 block이나 후속 산출물이
생긴 뒤에는 교체를 거부해 사람의 작업을 보존합니다.
...
| PDF 아이콘 검사 | 사용자가 선택한 block crop의 아이콘 판정과 원문 제안 | 자동 적용·미선택 block 검사 |
...
만들어 사람이 수정하거나 제외하도록 합니다. 원본 교체 가능 여부는
`source_registration_service`가 검수 block과 후속 작업물의 존재를 다시 검사해
결정합니다.

원문 검수 서버의 `POST /api/icon-audit`는 `pdf_icon_audit_service`를 호출합니다.
...
PDF 아이콘 검사와 translation provider는 각 작업의 prompt·응답 schema·결과
검증만 담당합니다.
...
- 이전 번역본 다운로드는 current가 아닌 기존 승인본 또는 프로젝트 ID와 이유를
  검증한 `translation_restart_*` 바로 아래 snapshot만 허용하고, 승인 state의
  경로와 SHA-256을 전송 직전에 다시 확인
