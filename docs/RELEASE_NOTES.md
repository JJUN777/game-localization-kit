...
대시보드와 세 검수 화면의 작업 흐름을 다듬고, OpenAI 지원과 AI 보조 검수,
단계별 비용 추적을 더한 기능 릴리즈입니다. 기존 프로젝트 구조를 유지하면서
원문 복구부터 용어 정리, 번역 프롬프트 작성과 최종 파일 관리까지 GUI에서 더
빠르고 명확하게 처리할 수 있습니다.
...
- 프로젝트별 AI 사용량 원장과 원문 추출·검수·용어 정리·번역·번역 검수의
  단계별 누적 예상 비용
- 프롬프트 변경 직후와 전체 재번역 뒤에도 기존 최종 승인 파일을 검증해 받을 수
  있고 저장 파일명에 승인 시각을 붙이는 대시보드 `이전 번역본` 이력
...
- 용어 검수의 수동 용어 추가를 필터 줄로 분리하고, 일괄 상태 변경 도구는 행을
  선택했을 때 필터 줄을 같은 높이로 교체해 선택 개수·선택 해제와 함께 표시하며,
  상태를 고르기 전에는 적용 버튼을 비활성화
...
- 번역 검수의 숫자·토큰·태그·용어·텍스트·미번역 의심 문제를 유형별 개수와
  함께 필터링하고, block 목록의 상태 도트에 오류·경고·수정 범례를 추가
- 현재 번역본·승인 원문·이전 번역본을 카드의 `파일 저장` 영역에 통합하고,
  번역 프롬프트는 `설정`, 완료 작업 상세는 접힌 `작업 기록`으로 정리
...
- 전체 unittest 372개
- 프로젝트 설정 범위 mypy 61개 파일과 전체 Python 소스 Ruff 검사
- 대시보드와 번역 검수 JavaScript 문법 검사
