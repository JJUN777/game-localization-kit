...
03_terminology/glossary_review.tsv    ← 사람이 검토하는 파일
        ...
        ↓ glk glossary import (또는 브라우저의 "용어집 확정")
...
- 아직 `review`인 자동 후보의 `후보 1차 정리(AI)` 미리보기·선택 반영
- 자동 후보를 보존한 채 수동 용어 행 추가·삭제
- 현재 TSV hash를 이용한 동시 편집 충돌 방지
- `검수 내용 저장` 후 `용어집 확정` 연속 실행
...
`후보 1차 정리(AI)`는 로컬 후보 생성과 별개의 선택 기능입니다. 실행 전 대상 수,
모델, 새 요청 수, 저장된 결과 수, 예상 token·비용을 보여줍니다. AI는 자동 후보 중
현재 상태가 `review`인 행만 승인·원문 유지·제외·직접 확인으로 추천하며, 수동
용어와 이미 상태를 결정한 행은 건드리지 않습니다. 낮은 신뢰도의 결과는 새 상태를
만들지 않고 `review`로 남깁니다. 실행 중에는 분석한 후보 수, API 요청 수, 저장된
결과 수, 현재 단계와 경과 시간을 표시합니다. 결과 창에서 추천과 이유를 확인한 뒤 반영하며,
반영해도 `검수 내용 저장` 전에는 TSV가 바뀌지 않습니다. 반영 직후에는 한 번
취소할 수 있고, 사람이 입력한 번역어·바꾼 분류는 덮어쓰지 않습니다.
원문 유지·제외 추천이 기존 번역어를 지우게 되는 경우에는 상태도 자동 반영하지
않고 `직접 입력 보호`로 표시합니다. 현재 번역어·분류·상태와 AI 추천이 다르면
결과 행에 두 값을 함께 보여주고 실제로 유지되는 값을 명시합니다. 충돌 행은
기본적으로 현재 입력을 유지하며, `AI 추천으로 변경`을 선택한 행만 AI 상태·번역어·
분류로 바꿉니다.

후보별 결과는 `.glk/state/glossary_ai_review.json`에 provider·model·prompt version과
입력 fingerprint를 함께 저장합니다. 입력과 설정이 같으면 다시 호출하지 않으며,
새 API 사용량만 `.glk/state/ai_usage.jsonl`의 `glossary` 단계에 누적합니다.
...
브라우저에서는 `용어집 확정` 시 원문에서 찾을 수 없는 수동 용어가 있을 때만
확인 창을 표시합니다. 목록을 검토한 뒤 `그대로 포함하고 확정`을 선택하면 같은
동작을 합니다.
