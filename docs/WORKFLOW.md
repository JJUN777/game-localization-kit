...
않습니다. 일부 원본만 실패하면 `일부 처리 실패`, 모든 원본이 실패하면
`실행 실패`로 구분합니다. 모델 없음, API 키, 권한, 사용량 한도와 네트워크
오류는 provider 원문 대신 키 값을 노출하지 않는 사용자용 안내로 표시합니다.
이전 버전에서 저장한 generic 실패 기록도 대시보드 시작 시 같은 기준으로
자동 보정합니다.
...
대시보드에서 연 번역 검수 화면은 최종 승인 성공 뒤 완료 모달을 표시합니다.
`대시보드로 돌아가기`를 선택하면 프로젝트 상태를 다시 읽어 최종 번역 완료
100% 상태와 출력 파일 생성을 확인할 수 있습니다. CLI에서 직접 연 화면은
독립 실행을 유지하며 복귀 버튼을 숨깁니다.
