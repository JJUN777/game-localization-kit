...
기본 제공자는 Gemini이며 기본 모델은 `gemini-2.5-flash`입니다. OpenAI의
...
PDF 프로젝트에서는 상단의 `PDF 아이콘 확인`으로 선택 모드에 들어갈 수
있습니다. 아이콘이 포함된 block을 체크하거나 현재 페이지의 block을 한꺼번에
선택한 뒤 `선택 블록 AI 검사`를 실행합니다. 검사 결과는 원본 crop, 현재 원문,
아이콘 token을 삽입한 제안을 나란히 표시합니다. `제안 적용`을 누르기 전에는
원문을 변경하지 않으며, 적용한 내용도 저장과 최종 승인을 거쳐야 합니다. 한 번에
최대 24개 block을 선택할 수 있습니다.
...
아이콘 검사 규칙은 프로그램에 고정되어 있습니다. 프로젝트의
...
생성에 실패하면 TSV 저장 여부와 함께 실패한 record·용어 및 해결 안내를 화면에
표시합니다. 원문에 없는 수동 용어는 철자를 확인하거나 화면 아래의 명시적 허용
항목을 선택한 뒤 다시 시도합니다.
...
확인합니다. 이 동작 자체는 AI API를 호출하지 않으며 내용은
...
`QA 오류 예외 승인`으로 최종 승인할 수 있습니다. block 누락·순서, token 또는
...
원문 검수를 최종 승인하면 프로젝트 카드의 현재 작업 버튼 아래에
`검수 완료 원문 TXT 저장`이 표시됩니다. 이 버튼은 승인된 원문 본문을 페이지별로
