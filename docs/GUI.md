...
기본 브라우저를 열지 않거나 경로·포트를 직접 정할 수 있습니다.
...
대시보드 주소는 실행한 터미널에 표시됩니다. 종료하려면 해당 터미널에서
...
관리하며 갱신 절차는 [Gemini 모델 목록 관리](GEMINI_MODELS.md)에 있습니다.
...
이미지를 등록하거나 교체할 때 모든 이미지에 적용할 OCR 프롬프트를 함께
확인합니다. 내용은 `01_input/images/ocr_prompt.txt`에 저장됩니다.
...
termbase 생성 뒤 카드의 `번역 프롬프트 설정`에서 프로젝트 공통 문체·표현
지침을 확인합니다. 이 동작 자체는 Gemini를 호출하지 않으며 내용은
...
- PDF 프로젝트: `<PDF 파일명>_kor.txt`
- 이미지 프로젝트: 이미지별 `<이미지 파일명>_kor.txt`
- 이미지 프로젝트 통합본: `combined_kor.txt`

GUI는 `05_output/`에 있는 파일 중 현재 승인 기록의 경로와 SHA-256이 일치하는
파일만 표시하고, 전송 직전 hash를 다시 확인합니다. 승인 뒤 파일을 외부에서
바꾸면 다운로드하지 않고 다시 확인하도록 안내합니다.
