...
완료했거나 현재 열린 원문·용어·번역 검수 단계는 파이프라인에서 직접 다시 열 수
있습니다. 원문 TXT는 `파일 저장`, 원본 교체와 OCR 프롬프트는 `설정` 메뉴에 모아 다음
작업과 구분합니다. 각 단계에는
...
원문 검수를 최종 승인하면 프로젝트 카드의 `파일 저장` 메뉴에
`검수 완료 원문 TXT 저장`이 표시됩니다. 이 메뉴 항목은 승인된 원문 본문을 페이지별로
