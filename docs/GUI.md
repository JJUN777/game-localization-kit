...
원문 추출·OCR을 시작하기 전에는 `설정 > 원본 교체`를 사용할 수 있습니다.
교체하면
...
변경은 이미지 등록 뒤 카드의 `설정 > OCR 프롬프트 수정`에서 별도로
진행합니다. 내용은
...
- 원본은 유지하고 지침만 바꾸려면 `설정 > OCR 프롬프트 수정`을 사용합니다.
...
- 이미지 OCR 시작 확인 창에는 이번 작업에 적용할 프롬프트를 읽기 전용으로
  표시합니다. 변경이 필요하면 창을 닫고 `설정 > OCR 프롬프트 수정`을
  사용합니다.
...
- `PDF 원문 준비 다시 실행`: 현재 원본으로 실패한 처리를 다시 실행합니다.
...
용어 검수를 최종 승인해 termbase가 최신 상태가 된 뒤에만 카드의 `설정` 메뉴에
`번역 프롬프트 설정`이 나타납니다. 한 번 저장한 뒤에는
`번역 프롬프트 수정`으로 표시됩니다. 여기에서
프로젝트 공통 문체·표현 지침을 확인합니다. 직접 편집하는 동작은 AI API를
호출하지 않으며 내용은
`04_translation/prompt.txt`에 저장됩니다.

초벌 번역 시작 확인 창은 현재 프롬프트가 기본값인지 저장한 설정인지 표시합니다.
바로 옆의 `수정`을 누르면 해당 프로젝트의 번역 프롬프트 편집 창으로 이동합니다.
...
- 발견된 문제가 있으면 `문제 유형`에서 숫자, 토큰·태그, 용어, 텍스트 오류,
  미번역 의심별 block 수를 확인하고 해당 유형만 표시
- 왼쪽 block 목록 위의 범례에서 오류·경고·수정 상태 도트의 의미 확인
...
`실행 중단`으로 바꿉니다. 실제 외부 요청을 자동으로 되살리지는 않으며 카드에
표시되는 단계별 다시 실행·다시 시도·이어서 실행을 사용해야 합니다. 번역은
마지막으로 확정한 청크
...
| `실행 중단` | 이전 대시보드가 작업 중 종료됨, 단계별 다시 실행 또는 이어서 실행 |
