...
> 상태: 2026-07-23까지 누적된 전체 체크리스트 기록입니다. 현재 범위는 [활성 README](../../source/README.md)를 기준으로 합니다.
...
현재 구현된 전체 실행 순서와 용어 기준은 [전체 작업 흐름](../../source/docs/WORKFLOW.md)을 사용합니다.
용어 후보의 편집 파일과 수동 추가·import 검증 규칙은 [용어집 검토 사양](../../source/docs/GLOSSARY.md)을 사용합니다.
PDF 다단·줄바꿈 복원 실험은 [PDF 레이아웃 복원 PoC](LAYOUT_RECONSTRUCTION_POC.md)를 참고합니다.

## 다음 작업

PDF·이미지 원문 검수와 승인 원문 기반 용어 후보 TSV 생성까지 완료됐습니다. 다음 우선 작업은 사람이 검토한 TSV를 검증해 최종 termbase로 가져오는 `glk glossary import`입니다.

1. TSV의 필수 컬럼, status, category, candidate ID와 중복을 검증한다.
2. 사람이 추가한 빈 candidate ID 행의 원문 위치·빈도·예문을 자동 보충한다.
3. 원문에 없는 선등록 용어는 `--allow-missing-terms`를 명시한 경우에만 허용한다.
4. `terminology/termbase.json`을 원자적으로 생성하고 승인 원문·TSV hash를 기록한다.
5. 확정 termbase를 번역 prompt와 QA에서 공통 사용하고 원문 block ID를 유지한 번역 세그먼트를 생성한다.

이 시점의 상세 상태와 검증 명령은 [작업 히스토리 보관본](WORK_HISTORY_20260723.md)을 참고합니다.
...
- [x] 최종 승인 원문에서 편집 가능한 glossary 후보 TSV를 생성한다.
- [ ] 검수 완료 TSV를 glossary JSON으로 변환하는 명령을 제공한다.
- [ ] config 또는 프로필 glossary로 안전하게 병합하는 명령을 제공한다.
- [x] 대소문자 변형과 보수적인 단수·복수형을 같은 후보군으로 묶는다.
- [x] 후보 TSV에 `review`, `approved`, `keep`, `rejected` 상태 컬럼을 둔다.
- [x] 후보 TSV에 출현 위치와 짧은 예문을 추가한다.
- [x] 기존 후보 TSV의 사람 편집 내용을 보존하고 stale 상태를 감지한다.
