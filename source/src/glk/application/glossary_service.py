...
def _collect_occurrences(
    ...
    for block in blocks:
        text = _TOKEN_PATTERN.sub(" ", block.effective_text)
        example = _clean_example(block.effective_text)
        block_type = block.block_type.casefold()
        for segment_index, segment in enumerate(_SEGMENT_SPLIT_PATTERN.split(text)):
            words = _WORD_PATTERN.findall(segment)
            if not words:
                continue
            whole_heading = block_type in _HEADING_TYPES and len(words) <= 6
