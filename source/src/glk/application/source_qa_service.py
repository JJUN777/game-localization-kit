...
SOURCE_QA_VERSION = "source-qa-local-v4"
...
def _block_issues(block: SourceBlock, allowed_tokens: set[str]) -> list[SourceQaIssue]:
    ...
    if block.source_hash != expected_hash:
        add("error", "SOURCE_HASH_MISMATCH", "raw_text does not match source_hash.")
    if block.legibility == "uncertain":
        add("error", "OCR_UNCERTAIN", "OCR marked this block as uncertain.")
    if block.warnings:
        add(
            "warning",
            "SOURCE_WARNING",
            "Source acquisition reported warnings for this block.",
            " | ".join(block.warnings),
        )
    if "[ILLEGIBLE]" in text.upper():
        add("error", "OCR_ILLEGIBLE", "Block contains an illegible text marker.")
    unresolved_icons = _UNRESOLVED_ICON_PATTERN.findall(text)
    if unresolved_icons:
        add(
            "warning",
            "ICON_UNRESOLVED",
            "Block contains an icon that was not mapped to a known token.",
            ", ".join(unresolved_icons),
        )
    if "�" in text:
        add("error", "UNICODE_REPLACEMENT", "Block contains a replacement character.")
    ...
    if "{" in token_stripped or "}" in token_stripped:
        add("error", "TOKEN_MALFORMED", "Block contains malformed token braces.")
    ...
    if allowed_tokens:
        for token in sorted(set(tokens) - allowed_tokens):
            add(
                "warning",
                "TOKEN_UNKNOWN",
                f"Token {{{token}}} is not defined in the OCR prompt.",
                f"{{{token}}}",
            )
    for filename_token in _IMAGE_FILENAME_PATTERN.findall(text):
        add(
            ...
            "Icon filename appears in OCR text instead of a braced token.",
            filename_token,
        )
    ...
    if ambiguous:
        add(
            "warning",
            "OCR_ALNUM_CONFUSION",
            "Alphanumeric text may contain O/0 or I/l/1 OCR confusion.",
            ", ".join(ambiguous),
        )
    ...
    if block.block_type.casefold() == "identifier" and not is_token_only:
        if not _IDENTIFIER_PATTERN.fullmatch(text):
            add(
                "warning",
                "IDENTIFIER_FORMAT",
                "Identifier contains unexpected whitespace or punctuation.",
            )
    if "\t" in text or re.search(r" {3,}", text):
        add(
            "info",
            "WHITESPACE_SUSPICIOUS",
            "Block contains tabs or three or more consecutive spaces.",
        )
    if len(text) > 5000:
        add(
            "warning",
            "BLOCK_TOO_LONG",
            "Block is unusually long and may contain merged reading-order content.",
        )
    if block.status == "flagged" and not any(
        ...
        add(
            "warning",
            "SOURCE_PRE_FLAGGED",
            "Source acquisition marked this block for review.",
        )
    return issues
        ...
        for block in matches:
            issues.append(
                _issue(
                    ...
                    message="Identifier appears in more than one source block.",
                    evidence=identifier,
                )
            )
